...
def productManager():
    ...
    return render_template(
        "productManager.html", book_data=book_data, user=current_user.name
    )


@login_required
def book():  # 這裡有加圖片了加的方式參照productManager.html
    mid = current_user.id
    ...
    for i in book_row:
        ...
        book = {
            ...
            "商品數量": i[5],
            "商品圖片": img_str,
            #   '商品圖片': f"data:image/png;base64,{image}" if image is not None else None,
        }
...
def news():
    ...
    news_data = []
    for i in news_row:
        news = {"消息標題": i[0], "發布內文": i[1], "發布時間": i[2]}
...
def show_new_info():
    ...
    news = {"消息標題": title, "發布內文": content, "發布時間": release}
    return news
...
def orderManager():
    if request.method == "POST":
        ...
        order_data = []
        for i in order_row:
            order = {"訂單編號": i[0], "訂購人": i[1], "訂單總價": i[2], "訂單時間": i[3]}
...
#########################################################################################
# @manager.route('/publish')
# def publish():
#    return redirect(url_for('manager.newsManager'))
...
#
#    return render_template('publish.html')
# @manager.route('/news/newsManager', methods=['GET', 'POST'])
# def newsManager():
# if request.method == 'GET':
#     if(current_user.role == 'user'):
#         flash('No permission')
...
# if 'delete' in request.values:
#     title = request.values.get('delete')
#     data = delete_News(title)

#     if(data != None):
#         flash('failed')
#     else:
#         data = News.get_news()
#         News.delete_News(title)

# elif 'edit' in request.values:
#     title = request.values.get('edit')
#     return redirect(url_for('manager.edit', title=title))

#  name = Product.get_MID_Product()
# existProduct = []
#  for i in name:
# existProduct.append(i[0])
#  if(current_user.id in existProduct):
# news_data = news()

#   mid = current_user.id
#   news_row = News.get_news()
#   news_data = []
#   for i in news_row:
#       news = {
#         '消息標題': i[0],

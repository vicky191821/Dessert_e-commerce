...
def bookstore():
    ...
    if 'keyword' in request.args and 'page' in request.args:
    ...
    elif 'categorymain' in request.args and 'page' in request.args:
        print('a')
        total = 0
        single = 2
        page = int(request.args['page'])
        ...
        return render_template('bookstore.html', single=single, categorymain=dessert_type, book_data=book_data, user=current_user.name, page=1, flag=flag, count=count)
